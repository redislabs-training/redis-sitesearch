...
async def test_transform_documents_elides_body_if_longer_than_max():
    ...
    docs = await transform_documents([doc], config.default_search_site, 'test', 5)
...
async def test_transform_documents_retains_body_if_shorter_than_max():
    ...
    docs = await transform_documents([doc], config.default_search_site, 'test')
...
async def test_transform_documents_injects_landing_page_doc():
    docs = await transform_documents([], config.default_search_site, 'rc')
    assert docs[0]['title'] == 'Redis Enterprise Cloud'

    docs = await transform_documents([], config.default_search_site, 'k8s*')
