...
def indexer():
    ...
    yield Indexer(DOCS_STAGING, mock_search_client)
...
def test_indexer_indexes_page_section_documents(index_file):
    ...
    for i, doc in enumerate(expected_section_docs, start=1):
        key = keys.document(DOCS_STAGING.url, doc['doc_id'])
...
def test_document_parser_extracts_one_page():
    ...
    """, 'html.parser')
    assert DocumentParser(DOCS_STAGING.validators).extract_hierarchy(
        html) == ["Platforms"]
