...
from docsearch.config import REDIS_LABS_DOCS_STAGING
...
TEST_URL = f"{REDIS_LABS_DOCS_STAGING.url}/test"


@pytest.fixture()
def indexer():
    mock_search_client = mock.MagicMock()
    yield Indexer(REDIS_LABS_DOCS_STAGING, mock_search_client)
...
def parse_file():
    ...
    def fn(filename):
        ...
        return DocumentParser(
            REDIS_LABS_DOCS_STAGING.validators).parse(TEST_URL, html)
...
def test_indexer_indexes_page_document(index_file):
    ...
    key = keys.document(REDIS_LABS_DOCS_STAGING.url, expected_doc['doc_id'])
...
def test_indexer_indexes_page_section_documents(index_file):
    ...
    print(indexer.search_client.redis.hset.call_args_list)
    for doc in expected_section_docs:
        key = keys.document(REDIS_LABS_DOCS_STAGING.url, doc['doc_id'])
...
def test_document_parser_extracts_one_page():
    ...
    """, 'html.parser')
    assert DocumentParser(REDIS_LABS_DOCS_STAGING.validators).extract_hierarchy(
        html) == ["Platforms"]
